...
static void hp_wmi_compat_remove_cb(void *data) { platform_profile_remove(); }

static struct device *
devm_platform_profile_register(struct device *dev, const char *name,
...
static void hp_wmi_compat_remove_int(void *data) { platform_profile_remove(); }
...
static const char *const omen_thermal_profile_boards[] = {
    ...
    "8917", "8918", "8949", "894A", "89EB", "8A15", "8A42", "8A43", "8BAD",
    "8C58", "8E41", "8BA9", "8BCA", "8D2F",
};
...
static inline u8 rpm_to_pwm(u8 rpm, u8 max_rpm) {
  return fixp_linear_interpolate(0, 0, max_rpm, U8_MAX,
                                 clamp_val(rpm, 0, max_rpm));
}

static inline u8 pwm_to_rpm(u8 pwm, u8 max_rpm) {
  return fixp_linear_interpolate(0, 0, U8_MAX, max_rpm,
...
static int hp_wmi_get_mux_supported_modes(u8 *supported) {
  ...
  ret = hp_wmi_perform_query(HPWMI_GET_SYSTEM_DESIGN_DATA, HPWMI_GM, buffer,
                             zero_if_sup(req_packet), sizeof(buffer));
  if (ret == 0) {
    *supported = buffer[7];
    return 0;
  }
...
static int hp_wmi_get_mux_mode(u8 *mode) {
  ...
  ret = hp_wmi_perform_query(HPWMI_GRAPHICS_MUX_QUERY, HPWMI_READ, buffer,
                             sizeof(buffer), sizeof(buffer));
  if (ret < 0)
    return ret;
  if (ret > 0)
...
static struct attribute *hp_wmi_attrs[] = {
    &dev_attr_display.attr,      &dev_attr_hddtemp.attr,
    &dev_attr_als.attr,          &dev_attr_dock.attr,
    &dev_attr_tablet.attr,       &dev_attr_postcode.attr,
    &dev_attr_gpu_mux_mode.attr, NULL,
};
  ...
  case hwmon_pwm:
    if (attr == hwmon_pwm_input) {
      if (hp_wmi_fan_control_supported()) {
        ...
        if (rpm < 0)
          return rpm;
        *val = rpm_to_pwm(rpm / 100, (channel == GPU_FAN) ? priv->gpu_max_rpm
                                                          : priv->cpu_max_rpm);
      } else {
...
static int hp_wmi_setup_fan_settings(struct hp_wmi_hwmon_priv *priv) {
  ...
  if (ret) {
    pr_info("HP WMI: Fan table query failed (%d), using cpu_max_rpm=%u, "
            "gpu_max_rpm=%u\n",
            ret, priv->cpu_max_rpm, priv->gpu_max_rpm);
  ...
  if (fan_table->header.num_fans == 0) {
    pr_info("HP WMI: Fan table reports 0 fans, using cpu_max_rpm=%u, "
            "gpu_max_rpm=%u\n",
  ...
  if (min_rpm == U8_MAX || (cpu_max_rpm == 0 && gpu_max_rpm == 0)) {
    pr_info("HP WMI: Fan table empty after parsing, using cpu_max_rpm=%u, "
            "gpu_max_rpm=%u\n",
            priv->cpu_max_rpm, priv->gpu_max_rpm);
    return 0;
  }
